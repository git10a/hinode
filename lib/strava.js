...
const STRAVA_ACCESS_TOKEN_REVALIDATE_SECONDS = 15 * 60;
...
async function refreshStravaAccessToken() {
    const res = await fetch(TOKEN_URL, {
        ...
        next: { revalidate: STRAVA_ACCESS_TOKEN_REVALIDATE_SECONDS },
    });
    if (!res.ok) {
...
const getCachedAccessToken = unstable_cache(
    ...
    ['strava-access-token'],
    { revalidate: STRAVA_ACCESS_TOKEN_REVALIDATE_SECONDS }
);

const getAccessToken = cache(getCachedAccessToken);
...
async function fetchGroupEvents(token, upcoming) {
    const clubId = process.env.STRAVA_CLUB_ID;
    const res = await fetch(
        `${API_BASE}/clubs/${clubId}/group_events?upcoming=${upcoming}&per_page=200`,
        {
            headers: { Authorization: `Bearer ${token}` },
            next: { revalidate: 3600 },
        }
    ...
    return parseWithBigIntIds(await res.text());
}

async function fetchGroupEventDetail(token, eventId) {
    const res = await fetch(`${API_BASE}/group_events/${eventId}`, {
        headers: { Authorization: `Bearer ${token}` },
        next: { revalidate: 3600 },
    });
    if (!res.ok) {
        console.error('Strava group_event fetch failed:', res.status);
        return null;
    }
    return parseWithBigIntIds(await res.text());
}

export async function getGroupEventCountData() {
    const clubId = process.env.STRAVA_CLUB_ID;
    ...
    try {
        ...
        const [upcomingEvents, pastEvents] = await Promise.all([
            fetchGroupEvents(token, true),
            fetchGroupEvents(token, false),
        ]);
        if (!upcomingEvents || !pastEvents) return null;

        const activeEventIds = [...new Set(upcomingEvents.map((event) => String(event.id)))];
        const details = await Promise.all(
            activeEventIds.map((eventId) => fetchGroupEventDetail(token, eventId))
        );
        if (details.some((event) => !event)) return null;

        return {
            activeEvents: details.map((event) => ({
                eventId: String(event.id),
                frequency: event.frequency || null,
                startDateTime: event.start_datetime || null,
                weeklyInterval: event.weekly_interval || 1,
                zone: event.zone || 'Asia/Tokyo',
                upcomingOccurrences: event.upcoming_occurrences || [],
            })),
            pastEvents: pastEvents.flatMap((event) =>
                (event.upcoming_occurrences || []).map((startAt) => ({
                    eventId: String(event.id),
                    startAt,
                }))
            ),
        ...
        console.error('Strava run count fetch error:', e.message);
        return null;
    }
}
...
export async function getUpcomingGroupEvents() {
    ...
    try {
        const token = await getAccessToken();
        const events = await fetchGroupEvents(token, true);
        if (!events) return [];
        const now = Date.now();
        const flat = [];
