...
export function getPostDisplayDate(post = {}) {
    return post.publicUpdatedAt || post.revisedAt || post.updatedAt || post.publishedAt || post.createdAt || '';
}
...
export function buildBlogContent(html) {
    ...
    // 記事タイトルはテンプレート側のh1で表示する。CMS本文に入ったh1は
    // ページ内の見出しとしてh2へ下げ、同じページにh1が重複しないようにする。
    const normalizedHeadings = html
        .replace(/<h1\b/gi, '<h2')
        .replace(/<\/h1>/gi, '</h2>');

    const withIds = normalizedHeadings.replace(/<(h2|h3)\b([^>]*)>([\s\S]*?)<\/\1>/gi, (match, tag, attrs, inner) => {
